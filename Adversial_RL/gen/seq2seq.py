...
def basic_rnn_seq2seq(
  ...
  with variable_scope.variable_scope(scope or "basic_rnn_seq2seq"):
    _, enc_state = static_rnn(cell, encoder_inputs, dtype=dtype)
...
def embedding_rnn_seq2seq(encoder_inputs,
  ...
  with variable_scope.variable_scope(scope or "embedding_rnn_seq2seq") as scope:
    ...
    # Encoder.
    encoder_cell = core_rnn_cell_impl.EmbeddingWrapper(
    ...
    if output_projection is None:
      cell = core_rnn_cell_impl.OutputProjectionWrapper(cell, num_decoder_symbols)
...
def embedding_tied_rnn_seq2seq(encoder_inputs,
  ...
  with variable_scope.variable_scope(
    ...
    if output_projection is None:
      cell = core_rnn_cell_impl.OutputProjectionWrapper(cell, output_symbols)
  ...
  with variable_scope.variable_scope(
    ...
    for name, decoder_inputs in decoder_inputs_dict.items():
      ...
      with variable_scope.variable_scope("one2many_decoder_" + str(
          name)) as scope:
        decoder_cell = core_rnn_cell_impl.OutputProjectionWrapper(cell,
                                                        num_decoder_symbols)
        if isinstance(feed_previous, bool):
          outputs, state = embedding_rnn_decoder(
