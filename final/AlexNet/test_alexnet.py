...
# load mean image of imagenet dataset
...
# placeholder for true labels for input images
...
h_conv_2_2 =\
  tf.nn.local_response_normalization(h_conv_2_2, depth_radius=radius,
                                     alpha=alpha, beta=beta, bias=bias)
h_pool_2_2 = tf.nn.max_pool(h_conv_2_2, ksize=[1, 3, 3, 1], strides=[1, 2, 2, 1],
                            padding="VALID")
...
h_conv_5_1 =\
  tf.nn.local_response_normalization(h_conv_5_1, depth_radius=radius,
                                     alpha=alpha, beta=beta, bias=bias)
h_pool_5_1 = tf.nn.max_pool(h_conv_5_1, ksize=[1, 3, 3, 1], strides=[1, 2, 2, 1],
                            padding="VALID")
...
h_conv_5_2 =\
  tf.nn.local_response_normalization(h_conv_5_2, depth_radius=radius,
                                     alpha=alpha, beta=beta, bias=bias)
h_pool_5_2 = tf.nn.max_pool(h_conv_5_2, ksize=[1, 3, 3, 1], strides=[1, 2, 2, 1],
                            padding="VALID")
...
im = np.expand_dims(imresize(imresize(imread(
  os.path.join("../imgs", args.image)), (256, 256))-mean, (224, 224)), axis=0)
